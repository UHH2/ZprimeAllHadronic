...
PreselectionHists::PreselectionHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("wjet_index", ";index;Events", 10, 0, 10);
...
if (!event.isRealData)
{
  ...
  if (has_gen_top)
  {
    ...
    {
      if (deltaR(the_gen_top,event.topjets->at(i))<0.8) hist("topjet_index")->Fill(i,weight);
    }
  }
  ...
  if (has_gen_w)
  {
    ...
    for (unsigned int i=0; i<event.topjets->size();i++)
    {
      if (deltaR(the_gen_w,event.topjets->at(i))<0.8) hist("wjet_index")->Fill(i,weight);
    }
  }
...
SelectionHists::SelectionHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("antitpmassCRbtag_zprimemass", ";m_{Z'};Events", 400, 0, 4000);
  book<TH1F>("antitpmassCRnobtag_zprimemass", ";m_{Z'};Events", 400, 0, 4000);
  ...
  //tp
  book<TH1F>("bkgtp1", ";m_{T'};Events", 400, 0, 4000);
  book<TH1F>("bkgtp2", ";m_{T'};Events", 400, 0, 4000);

  book<TH1F>("bkgtp1_par", ";m_{T'};Events", 400, 0, 4000);//
  book<TH1F>("bkgtp2_par", ";m_{T'};Events", 400, 0, 4000);//

  book<TH1F>("bkgtp1up_par_fit", ";m_{T'};Events", 400, 0, 4000);//
  book<TH1F>("bkgtp2up_par_fit", ";m_{T'};Events", 400, 0, 4000);//

  book<TH1F>("bkgtp1down_par_fit", ";m_{T'};Events", 400, 0, 4000);//
...
//selection
  ...
  //toptag+wtag
  TopJet the_top, the_w, the_w1, the_w2, the_top15, the_w15;
  Jet the_b, the_b2;
  Jet the_b_low;
...
int index_wtag12 = 0;
  ...
  for(auto fatjet : *fatjets)
  {
    if (TopTag15(fatjet))
    {
      for(auto topjet : *event.topjets)
      {
        if (WTag(topjet) && deltaR(topjet,fatjet)>0.8) 
        {
          has_tw15=true;
          the_top15=fatjet;
          the_w15=topjet;
  ...
  if (has_tw15 && !has_tw)
  {
    ...
    if (has_b)
    {
      float ht_twb = TopJetPt(the_top15)+TopJetPt(the_w15)+the_b.pt();
      float tprimemass=TprimeMass(the_w15,the_b);
      float zprimemass=ZprimeMassVLQ(the_top15,the_w15,the_b);
    ...
    if (has_b_low)
    {
      float ht_twb = TopJetPt(the_top15)+TopJetPt(the_w15)+the_b_low.pt();
  ...
  if (has_ww && !has_tw)
  {
    ...
    if (has_b && !has_b2) for(auto jet : *event.jets)
      if (deltaR(jet,the_w1)>DELTA_R &&deltaR(jet,the_w2)>DELTA_R && jet.pt()>50.0 && fabs(jet.eta()-the_b.eta())>0.01)
      {
    ...
    if (has_b && has_b2)
    {
      std::vector<TopJet> TopW =    {the_w1,  the_w2, the_w1,  the_w2};
      std::vector<TopJet> TprimeW = {the_w2, the_w1,  the_w2, the_w1};
...
if (has_ww)
{
  ...
  if (has_b && has_b2)
  {
    std::vector<TopJet> TpW  =   {the_w1,  /*the_w2,*/ the_w1/*,  the_w2*/};
    std::vector<TopJet> TpW2 =   {the_w2, /*the_w1,*/  the_w2/*, the_w1*/};
...
///////////////////end of ttbar ANALYSIS
  ...
  if (has_twb)
  {
      ...
      else
      {
      	if (topmaxcsv>medium_btag)
        {
        	hist("antitpmassCRbtag_zprimemass")->Fill(zprimemass,weight);
        }
        else
        {
        	hist("antitpmassCRnobtag_zprimemass")->Fill(zprimemass,weight);
        }
      }
      if (tprimemass>140.0 && tprimemass<250.0)
      {
  ...
  switch(choice)
  {
    case 0: tag_index=0; probe_index=1; break;
    case 1: tag_index=1; probe_index=0; break;
    case 2: tag_index=0; probe_index=2; break;
  ...
  if (has_twantibcsv)
  {
    ...
    float tprimemass=TprimeMass(the_w,the_b_antibcsv);
    float ht_twb = TopJetPt(the_top)+TopJetPt(the_w)+the_b_antibcsv.pt();
    if (tprimemass>500.0)
    {
  ...
  if (has_twantibcsvloose)
  {
    float zprimemass=ZprimeMassVLQ(the_top,the_w,the_b_antibcsvloose);
    hist("antibcsvlooseCR_zprimemass")->Fill(zprimemass,weight);
  ...
  if (has_twantibcsv15)
  {
    ...
    if (tprimemass>500.0)
    {
      if (getMaxCSV(the_top15)>medium_btag)
      {
        ...
        hist("bkgfat2")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "2", "nominal"));
        hist("bkgfat2_par")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "2par", "nominal"));//
        hist("bkgfat2up_par_fit")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "2par", "up_fit"));//
        hist("bkgfat2down_par_fit")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "2par", "down_fit"));//
      }
      else
      {
        ...
        hist("bkgfat1")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "1", "nominal"));
        hist("bkgfat1_par")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "1par", "nominal"));//
        hist("bkgfat1up_par_fit")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "1par", "up_fit"));//
        hist("bkgfat1down_par_fit")->Fill(zprimemass,weight*QCDWeightFat( zprimemass, "1par", "down_fit"));//
      }
    }
  }
